...
def tombol():
    global sheet, text_error, text2, text3, text4, text5, text_loc, text6, data_kategori, text7
...
def pengecekan_nik():
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row = 2, values_only = True), start=2):
        if str(row[0]) == str(nik): #Mengecek apakah nik yang diinput user ada di file
            ...
            text6 = tk.Label(windows, text="Kategori : " + str(row[4]))
        ...
        for i in row:
            if isinstance(i, str):
                ...
                row_baru.append(i)
        if str(row_baru[4]) == var_kategori:
